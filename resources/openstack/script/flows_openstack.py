...
def add_forward(selEv, res):
    #Except for the initial event, pending contains 
    # only events which are destination of interactions
    pending = [selEv]
    for firstEv in pending:
        #one iteration of this loop adds to res 
        #all the events pertaining to the block
        #in this entity, and to pending all the dst
        #of outgoing interactions
        l = firstEv.getLabel()
        toks = l.split()
        if toks[0] in ('GET' ,'PUT', 'POST', 'DELETE', 'PATCH'):
            req = l
        else:
            req = None
        ent = firstEv.getEntity()
        limit = ent.getLastEventIndex()
        for idx in range(firstEv.getIndex()+1, limit+1):
            ev = Main.getModel().getEventAt(idx)
            if ev.getEntity() != ent:
                continue
            if ev != firstEv:
                print "not first"
                if ev.isBlockBegin() or (not req) and len(ev.getIncomingInteractions()) > 0:
                    print "break 1: ",ev.getLabel(),  ev.isBlockBegin(), len(ev.getIncomingInteractions())
                    break        
            ...
            for inter in inters:
                ...
                if toEv:
                    if not req or ev.getLabel() == 'response('+req+')':
                        pending.append(toEv)
                        req = None
