...
public class LaneRenderer extends InteractionRenderer {
    ...
    public void setup(JSonObject props, Event ev) {
        ...
        if (getInteraction().getFromEvent() == ev) {
            ...
            if (props != null) {
                final JSonValue jpairingId = props.get("pairing_id");
                if (jpairingId != null) {
                    final String pairingId = jpairingId.toString();
                    for (int i = 0; i < MAX_LANES; i++) {
                        if (lanes[i] == null || lanes[i].equals(pairingId)) {
                            lanes[i] = pairingId;
                            lane = i;
                            if (lane > MAX_LANES / 10)
                                System.err.println("WARNING: Entity "
                                        + en.getPath() + "  using more than "
                                        + lane + " lanes.");
                            return;
                        }
                    }
    ...
    private int computePoints(int[] x, int y[], Rectangle b1, Rectangle b2) {
        if (b1.x < 0) {
            ...
            final int v1x = b1.x + b1.width / 2;
            final int v2x = b2.x + b2.width / 2;
    ...
    public void render(Rectangle b1, Rectangle b2, Graphics2D g2d,
        ...
        final int count = computePoints(xp, yp, b1, b2);
    ...
    public boolean inSelectionArea(Rectangle b1, Rectangle b2, int px, int py,
        ...
        for (int i = 0; i < count - 1; i++) {
            if (xp[i] == xp[i + 1]
                    && Math.abs(px - xp[i]) < 4
                    && ((py >= yp[i] && py <= yp[i + 1]) || (py <= yp[i] && py >= yp[i + 1])))
                return true;
            if (yp[i] == yp[i + 1]
                    && Math.abs(py - yp[i]) < 4
                    && ((px >= xp[i] && px <= xp[i + 1]) || (px <= xp[i] && px >= xp[i + 1])))
                return true;
        }
        return false;
    }
