...
public class DefaultInteractionRenderer extends InteractionRenderer {
    ...
    public void setup(JSonObject props, Event ev) {
        ...
        if (props != null) {
            ...
            final JSonValue stroke = props.get("stroke_width");
            if (stroke != null)
                strokeWidth = Float.parseFloat(stroke.toString());
            drawTip = !"false".equals(props.get("draw_tip"));
            dashed = "true".equals(props.get("dashed"));
            final JSonValue col = props.get("color");
            if (col != null) {
                final int c = Integer.parseInt(col.toString(), 16);
                color = new Color(c);
            }
        }
        final int sw = (int) strokeWidth;
        if (!dashed) {
    ...
    private void drawArrow(Graphics2D g2d, int x1, int y1, int x2, int y2,
        ...
        if ((!self) || (straight)) {
            ...
            if (drawTip) {
                ...
                final int x3 = (int) (x2 + ARROW_TIP_SIDE
                        * Math.cos(theta - ARROW_TIP_WIDTH));
                final int y3 = (int) (y2 + ARROW_TIP_SIDE
                        * Math.sin(theta - ARROW_TIP_WIDTH));
                final int x4 = (int) (x2 + ARROW_TIP_SIDE
                        * Math.cos(theta + ARROW_TIP_WIDTH));
                final int y4 = (int) (y2 + ARROW_TIP_SIDE
                        * Math.sin(theta + ARROW_TIP_WIDTH));
                g2d.fillPolygon(new int[] { x2, x3, x4 }, new int[] { y2, y3,
                        y4 }, 3);
            ...
            g2d.draw(new Arc2D.Float(x1 - ARC_BB_WIDTH / 2, y1, ARC_BB_WIDTH,
                    y2 - y1 + 1, -90, 180, Arc2D.OPEN));
            if (drawTip) {
    ...
    public void render(Rectangle b1, Rectangle b2, Graphics2D g2d,
        ...
        // a bounding box where x = -1 indicates the event is either not there,
        // or the entity for
        // the event is not being displayed
        // a null event indicates the event matching the other one is not in the
        // data set
        final Event fev = getInteraction().getFromEvent();
        final Event tev = getInteraction().getToEvent();
        final boolean self = (fev != null && tev != null && fev.getEntity() == tev
                .getEntity());
    ...
    private double pointSegmentDistance(int cx, int cy, Segment s) {
        final double r_numerator = (cx - s.x1) * (s.x2 - s.x1) + (cy - s.y1)
                * (s.y2 - s.y1);
        final double r_denomenator = (s.x2 - s.x1) * (s.x2 - s.x1) + (s.y2 - s.y1)
                * (s.y2 - s.y1);
        final double r = r_numerator / r_denomenator;
        final double ss = ((s.y1 - cy) * (s.x2 - s.x1) - (s.x1 - cx) * (s.y2 - s.y1))
                / r_denomenator;

        final double distanceLine = Math.abs(ss) * Math.sqrt(r_denomenator);
        double distanceSegment;
        if ((r >= 0) && (r <= 1)) {
            distanceSegment = distanceLine;
        } else {

            final double dist1 = (cx - s.x1) * (cx - s.x1) + (cy - s.y1)
                    * (cy - s.y1);
            final double dist2 = (cx - s.x2) * (cx - s.x2) + (cy - s.y2)
                    * (cy - s.y2);
            if (dist1 < dist2) {
    ...
    public boolean inSelectionArea(Rectangle b1, Rectangle b2, int px, int py,
        ...
        if ((!self) || straight) {
            ...
            final Arc2D a2din = new Arc2D.Float(s.x1 - ARC_BB_WIDTH / 2 + 3,
                    s.y1 + 3, ARC_BB_WIDTH - 6, s.y2 - s.y1 + 1 - 6, -90, 180,
                    Arc2D.OPEN);
            final Arc2D a2dout = new Arc2D.Float(s.x1 - ARC_BB_WIDTH / 2 - 3,
                    s.y1 - 3, ARC_BB_WIDTH + 6, s.y2 - s.y1 + 1 + 6, -90, 180,
                    Arc2D.OPEN);
            final Rectangle r1 = new Rectangle(px, py, 5, 5);
