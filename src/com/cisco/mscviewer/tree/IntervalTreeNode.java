...
public class IntervalTreeNode extends AVLTreeNode {
    ...
    private void fixMinStart() {
        final Interval in = (Interval) data;
    ...
    protected void getContainingIntervals(int c, ArrayList<Interval> ret) {
        final ArrayList<Interval> al = ret;
        final Interval ite = (Interval) data;
        final int s = ite.getStart();
        final int e = ite.getEnd();
        if (c >= s && c <= e)
            al.add(ite);
    ...
    public ArrayList<Interval> getIntersectingIntervals(int start, int end) {
        final ArrayList<Interval> els = new ArrayList<Interval>();
        getIntersectingIntervals(start, end, els);
        return els;

    }

    public void getIntersectingIntervals(int start, int end,
            ArrayList<Interval> ret) {
        final Interval ite = (Interval) data;
        if (end < minStart)
            return;
        ...
        if (right != null) {
            ((IntervalTreeNode) right)
                    .getIntersectingIntervals(start, end, ret);
        }
    }
    ...
    protected void verifyIntegrity(String path) throws TreeIntegrityException {
        ...
        final int end = in.getEnd();
        if (start < 0 && end < 0)
            throw new TreeIntegrityException("Invalid start-end", path,
                    data.getValue());
        if (left != null) {
            final Interval lin = (Interval) left.data;
            final String lx = "-L->" + left.toString();
            if (debug) {
                // disabled temporarily
                if (lin.getStart() < minStart) {
                    throw new TreeIntegrityException(
                            "left minstart error left.start=" + lin.getStart()
                                    + ", minStart=" + minStart, path + lx, left
                                    .getData().getValue());
                }
                final int lmin = ((IntervalTreeNode) left).minStart;
                if (lmin < minStart) {
                    throw new TreeIntegrityException(
                            "left minstart error left.minstart=" + lmin
                                    + ", minStart=" + minStart, path + lx, left
                                    .getData().getValue());
                }
            }
            if (lin.getEnd() > in.getEnd()) {
                throw new TreeIntegrityException("left end error: left.end="
                        + lin.getEnd() + ", this.end=" + in.getEnd(),
                        path + lx, left.getData().getValue());
            }
        }
        if (right != null) {
            final String rx = "-R->" + right.toString();
            final Interval rin = (Interval) right.data;
            if (debug) {
                // disabled temporarily
                if (rin.getStart() < minStart)
                    throw new TreeIntegrityException(
                            "right minstart error this=" + nodeRep()
                                    + ", right=" + right.nodeRep(), path + rx,
                            right.getData().getValue());
            }
            if (rin.getEnd() < in.getEnd())
                throw new TreeIntegrityException("right end error", path + rx,
                        right.getData().getValue());
        }
    }

    protected final void getIntervalsWithStartBound(int c,
            ArrayList<Interval> ret) {
        final ArrayList<Interval> al = ret;
        final Interval ite = (Interval) data;
        final int ite_start = ite.getStart();
        final int ite_end = ite.getEnd();
        if (c == ite_start)
            al.add(ite);
        if (c < minStart)
            return;
    ...
    public String nodeRep() {
        final Interval d = (Interval) data;
    ...
    public static void main(String args[]) {
        ...
        final IntervalTree t = new IntervalTree("test");
        t.add(new Info(l, l + r));
        long t1 = System.currentTimeMillis();
        final int NUM_INTERVALS = 1000000;
        final int NUM_POINT_QUERIES = 100000;
        final int NUM_INT_QUERIES = 100000;
        try {
            final FileWriter fw = new FileWriter("c:/temp/x");
            ...
            System.out.println(NUM_INTERVALS + " intervals populated in "
                    + ((t2 - t1) / 1000.0) + "s");
            // POINT QUERIES
            final int[] points = new int[NUM_POINT_QUERIES];
            ...
            System.out.println(NUM_POINT_QUERIES
                    + " point-queries executed in " + ((t2 - t1) / 1000.0)
                    + "s");
            for (int i = 0; i < NUM_POINT_QUERIES; i++) {
                fw.write(points[i] + ":\n");
            ...
            final int[] left = new int[NUM_INT_QUERIES];
            final int[] right = new int[NUM_INT_QUERIES];
            ...
            t2 = System.currentTimeMillis();
            System.out.println(NUM_INT_QUERIES + " int-queries executed in "
                    + ((t2 - t1) / 1000.0) + "s");
            for (int i = 0; i < NUM_INT_QUERIES; i++) {
