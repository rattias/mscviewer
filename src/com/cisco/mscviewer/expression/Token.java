...
package com.cisco.mscviewer.expression;
 public class Token {
     ...
     //	};
     enum TT {
         STRING(null), NUM(null), OPEN("("), CLOSE(")"), GEQ(">="),
         LEQ("<="), GT(">"), LT("<"), EQ("="), OR("or"), AND("and"),
         NOT("not"), LABEL("label"), TIME("time"), DURATION("duration"),
         STARTSWITH("startswith"), ENDSWITH("endswith"), CONTAINS("contains"),
         UNKNOWN(null), TYPE("type"), SOURCE("source"), SINK("sink"), 
         OPEN_SQUARE("["), CLOSE_SQUARE("]"), NS("ns"), US("us"), MS("ms"), S("s"); 
         String s;
     ...
     public void printRPN() {
         ...
         if (l != null) l.printRPN();
         if (r != null) r.printRPN();
     }
