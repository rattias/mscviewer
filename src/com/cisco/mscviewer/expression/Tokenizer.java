...
class Tokenizer {
    ...
    public static void main(String args[]) {
        final Tokenizer t = new Tokenizer(args[0]);
        t.print();
    }
    ...
    public Tokenizer(String txt) {
        ...
        outer: while (true) {
            ...
            // add known token
            for (final Token.TT tt : Token.TT.values()) {
                final String str = tt.toString();
                if (str != null && txt.substring(start).startsWith(str)) {
                    final int toklen = str.length();
                    if (!Character.isLetter(str.charAt(0))
                            || start + toklen == len
                            || (!Character.isLetter(txt.charAt(start + toklen)) && txt
                                    .charAt(start + toklen) != Token.COMPL_CHAR)) {
            ...
            if (Character.isDigit(txt.charAt(start))) {
                ...
                final String s = txt.substring(start, end);
            ...
            if (txt.charAt(start) == '"') {
                ...
                if (end == len) {
                    tokens.add(new Token(Token.TT.UNKNOWN, txt.substring(start,
                            end)));
                    break;
                } else {
                    end++;
                    tokens.add(new Token(Token.TT.STRING, txt.substring(start,
                            end)));
                    start = end;
                    continue;
                }
